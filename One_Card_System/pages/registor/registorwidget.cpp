...
QString userId_registor;
QString userPhone_registor;
...
//    connect(serial,SIGNAL(receivedMsg(QByteArray)),this,SLOT(onDecodeFrame(QByteArray)));
    uuid = new QUuid();
}
...
void RegistorWidget::on_btn_Register_clicked()
{
    ...
    QString personRemark = ui->lineEdit_PersonMark->text();
    QString cardRemark = ui->textEdit_CardMark->toPlainText();
    QString cardId = ui->lineEdit_CardId->text();
    QString userPhone = ui->lineEdit_Phone->text();
    QString userId = ui->lineEdit_studentID->text();
    userId_registor = ui->lineEdit_studentID->text();
    userPhone_registor = ui->lineEdit_Phone->text();
    ...
    if(!personTableModel->insertRecords(personId,cardId,userName,userId,userType,userPhone,personRemark))
    {
