...
extern QString userId_registor;
extern QString userPhone_registor;
...
void DialogCardConfig::onDecodeFrame(QByteArray bytes)
{
    ...
    if(frame.cmd.remove(" ") == "0702" && frame.status == "00")//授权成功
    {
        ...
        case 0: //init
        {
            ...
            buffer[0] = Packet_Pos;//绝对块号,这样的最好定义成宏,方便修改
            memset(buffer+1, 0, 5);
            //初始化金额
    ...
    else if(frame.cmd.remove(" ") == "0802" && frame.status == "00")//这里为什么不需要看是否成功
    {
