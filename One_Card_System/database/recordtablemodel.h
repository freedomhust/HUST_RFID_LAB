...
public:
    ...
    //根据卡号查找记录，返回块号
    int findRecordByID(const QString &tagId);

    //根据tagId删除记录
    bool deleteByTagId(const QString &tagId);

    //添加记录
    int addRecord(QString &tagId,QString &time,QString &addr,QString &readerId,QString &value, QString &phone,QString remark,QString block_num);
