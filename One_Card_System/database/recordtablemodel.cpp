...
/**
 * @brief RecordTableModel::findRecordByID
 * @param tagId
 * @return int型数据
 * 根据卡号查找记录
 */
...
int RecordTableModel::addRecord(QString &tagId, QString &time, QString &addr,
                                 QString &readerId, QString &value, QString &phone,QString remark,QString block_num)
{
    ...
    record.setValue(5, QVariant(phone));
    record.setValue(6, QVariant(remark));
    record.setValue(7, QVariant(block_num));
