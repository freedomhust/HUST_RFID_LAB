...
void MainWindow::handConnect()
{
    ...
    connect(ui->action_lostandfound,SIGNAL(triggered(bool)),this,SLOT(LostAndFound()));
...
/**
 * @brief MainWindow::LostAndFound
 * 挂失卡
 */
void MainWindow::LostAndFound()
{
    if(!CheckLogin())
        return;
    lostandfound *LostAndFound = new lostandfound(this,serialPortThread);
    connect(this,SIGNAL(sendCardId(QString)),LostAndFound,SLOT(on_tagIdReceived(QString)));
    LostAndFound->setWindowTitle(tr("用户挂失"));
    LostAndFound->exec();
    delete LostAndFound;
}
